...
async function checkOpenAICompat(
  ...
  apiKey: string,
  model: string,
): Promise<Omit<KeyCheckResult, 'providerId'>> {
  ...
  // 400 = key accepted but params rejected → still proves the key is valid
  if (res.ok || res.status === 400) return { status: 'ok', latencyMs };
  const txt = await res.text().catch(() => '');
  return { status: 'error', latencyMs, error: `HTTP ${res.status}: ${txt.slice(0, 120)}` };
}
...
export async function validateKey(providerId: string, apiKey: string): Promise<KeyCheckResult> {
  ...
  try {
    if (PEKPIK_PROVIDERS.has(providerId)) {
      const r = await checkOpenAICompat(
        `${PEKPIK_BASE_URL}/chat/completions`,
        apiKey,
        TEST_MODEL[providerId] ?? 'smart-chat',
      );
      return { ...r, providerId };
    }
...
/**
 * Parse the README markdown and return the list of free keys.
 *
 * The README uses a markdown table with the following columns:
 *   | Key | Model | Status | Budget | Rate | Expires | Description |
 *
 * The previous regex was broken (missing backslash escapes for `\|` and `\s`),
 * so the function silently returned 0 keys. The implementation below uses a
 * line-based parser that is far more tolerant of column variations and that
 * still works if the README schema drifts a little.
 */
export async function fetchFreeLLMKeys(): Promise<FreeLLMKey[]> {
  ...
  for (const rawLine of text.split('\n')) {
    ...
    // Quick filter: keep lines that look like a markdown table row containing an sk- key
    if (!line.startsWith('|')) continue;
    if (!/`sk-[A-Za-z0-9_-]{10,}`/.test(line)) continue;

    // Split table cells, drop the leading/trailing empty parts produced by the surrounding pipes
    const cells = line.split('|').slice(1, -1).map((c) => c.trim());
    if (cells.length < 2) continue;

    // First cell holds the backtick-wrapped key
    const keyMatch = cells[0].match(/`(sk-[A-Za-z0-9_-]+)`/);
