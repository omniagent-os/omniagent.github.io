...
function FreeLLMDrawer({
  ...
  keys: FreeLLMKey[];
  onPick: (providerId: string, key: string) => void;
}) {
        ...
        {open && (
          <motion.div initial={{ height: 0 }} animate={{ height: "auto" }} exit={{ height: 0 }} className="overflow-hidden">
            <div className="p-3 border-t border-border space-y-3">
              <Input
                placeholder="Filtrer par modèle ou clé…"
                value={filter}
              ...
              <ScrollArea className="h-52">
                <div className="space-y-1 pr-2">
                  {filtered.length === 0 && (
                    <p className="text-xs text-muted-foreground text-center py-4">Aucune clé trouvée.</p>
                  )}
                  {filtered.map((k, i) => {
                    // Determine which providers can use this key
                    const targets = pekpikProviders.filter((pid) =>
                      (modelMap[pid] ?? []).some(
                        (m) => k.model.toLowerCase().includes(m.split("-")[0]) || m === k.model,
                      ) || k.model === "smart-chat",
                    );
                    return (
...
export default function ApiKeys() {
  ...
  const checkKey = useCallback(async (providerId: string, apiKey: string) => {
    if (!apiKey) {
      setStatuses((s) => ({ ...s, [providerId]: { status: "missing" } }));
      return;
    }
  ...
  const handleRefreshFreeLLM = async () => {
    ...
    try {
      ...
      setFreeLLMKeys(keys);
      if (keys.length === 0) {
        toast.warning("Aucune clé extraite depuis le README — réessayez plus tard.");
      } else {
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            ...
            <Button
              ...
              onClick={handleRefreshFreeLLM}
              disabled={fetchingFreeLLM}
            >
          ...
          <p className="text-xs text-muted-foreground">
            ...
            <a
              href="https://github.com/alistaitsacle/free-llm-api-keys"
              target="_blank"
              rel="noreferrer"
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <Button
                      ...
                      variant="ghost"
                      className="h-6 px-2 text-xs"
                      onClick={() => checkKey(provider.id, provider.apiKey)}
                      disabled={!provider.apiKey || st?.status === "checking"}
                    >
                ...
                {st?.status === "error" && st.error && (
                  <div className="text-xs text-red-400 bg-red-500/5 border border-red-500/20 rounded-lg p-2">
                    <button
                      ...
                      onClick={() => setExpandedError(expandedError === provider.id ? null : provider.id)}
                    >
                  ...
                  <div className="flex flex-wrap gap-1">
                    {models.map((m) => (
                      <Badge
                        key={m}
                        variant="outline"
                        className={cn("text-xs border-border", m === provider.model && "border-primary/40 text-primary")}
                      >
                        {m === provider.model && <Clock className="w-3 h-3 mr-1" />}
                        {m}
                      </Badge>
                    ))}
                  </div>
