import express from "express";
  ...
  app.post("/api/proxy", async (req, res) => {
    ...
    try {
      ...
      switch (providerId) {
        ...
        case "openai":
          content = await callOpenAICompat("https://api.openai.com/v1/chat/completions",       process.env.OPENAI_API_KEY,    "openai",    model, messages, null); break;
        case "mistral":
          content = await callOpenAICompat("https://api.mistral.ai/v1/chat/completions",        process.env.MISTRAL_API_KEY,   "mistral",   model, messages, null); break;
        case "deepseek":
          content = await callOpenAICompat("https://api.deepseek.com/chat/completions",         process.env.DEEPSEEK_API_KEY,  "deepseek",  model, messages, null); break;
        case "groq":
          content = await callOpenAICompat("https://api.groq.com/openai/v1/chat/completions",   process.env.GROQ_API_KEY,      "groq",      model, messages, null); break;
        case "cerebras":
          content = await callOpenAICompat("https://api.cerebras.ai/v1/chat/completions",       process.env.CEREBRAS_API_KEY,  "cerebras",  model, messages, null); break;
        case "anthropic":
        ...
        case "pekpik":
          content = await callOpenAICompat(PEKPIK_BASE + "/chat/completions", process.env.PEKPIK_API_KEY, "pekpik", model, messages, PEKPIK_BASE); break;
        case "xai":
          content = await callOpenAICompat(PEKPIK_BASE + "/chat/completions", process.env.XAI_API_KEY,   "xai",   model, messages, PEKPIK_BASE); break;
        case "kimi":
          content = await callOpenAICompat(PEKPIK_BASE + "/chat/completions", process.env.KIMI_API_KEY,  "kimi",  model, messages, PEKPIK_BASE); break;
  ...
  /* ─── Provider helpers ────────────────────────────────── */
  async function callOpenAICompat(endpoint, apiKey, name, model, messages, _baseUrl) {
